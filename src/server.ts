...
/**
 * Per-model onPayload: ClinePass wire-model rewrite, then the generic
 * models.json wire-model rewrite (id -> wireModel). Both are no-ops for
 * models that don't use them, so this is safe to pass for every request.
 */
function combinedOnPayload(model: { provider?: string; id: string; wireModel?: string }): (payload: unknown, m: { id: string }) => unknown | undefined {
	...
	if (!cline) return (payload) => withWireModel(payload, model) ?? payload;
	return (payload, m) => {
		const next = cline(payload, m as any) ?? payload;
